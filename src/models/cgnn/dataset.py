...
class SantosTrainDatasetTorch(SantosTrainDataset, TorchDataset):
    ...
    def __getitem__(self, idx) -> tuple[AsyncMTSWindowPair, torch.Tensor]:
        ...
        if self.normalize:
            ...
            for ts_name in target_features.keys():
        ...
        empty_ts = []
        for ts_name in self.target_window_lengths.keys():
            context_ts = context_timestamps[ts_name]
            if len(context_ts) == 0:
                empty_ts.append(True)
            else:
                empty_ts.append(False)
        if all(empty_ts):
            return self.__getitem__(idx)
        else:
            return (x, y), t_inf
